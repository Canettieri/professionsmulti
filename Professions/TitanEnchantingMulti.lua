...
local function OnUpdate(self, id)
    ...
    if prof1 ~= nil then
        ...
        if skillLine == 333 then
            ...
            profOffset = offset
            if not startskill then
                startskill = skillLevel
            end
...
local function GetButtonText(self, id)
    local ENCMtext
    local bonusText = ""
    if ENCMIncrease and ENCMIncrease > 0 then
        -- Bônus da profissão
...
local function GetTooltipText(self, id)
    ...
    elseif (ENCM - startskill) < 0 then
        -- Segurança quando existe mudança de exp.
        ColorValueAccount = ""
    end
...
L.Elib({
    ...
    icon = (expansionLevel >= 9) and "Interface\\Icons\\ui_profession_enchanting" or "Interface\\Icons\\trade_engraving",
    category = "Profession",
    version = version,
    onUpdate = OnUpdate,
    onClick = OnClick,
