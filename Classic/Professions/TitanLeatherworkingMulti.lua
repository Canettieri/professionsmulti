...
local function GetButtonText(self, id)
	...
	elseif LEAM == LEAMmax then
		...
		if LEAMmax == maxCap then
			LEAMtext = "|cFF69FF69" .. L["maximum"] .. "!" .. SimpleText
		else
			LEAMtext = "|cFFFFFFFF" .. LEAM .. "|cFF69FF69! [" .. L["maximum"] .. "]" .. SimpleText .. BarBalanceText
		end
	else
...
local function GetTooltipText(self, id)
	local BonusTooltip = "\n"..L["craftsmanship"].."|r|cFFFFFFFF"..LEAM.." + |r|cFF69FF69"..LEAMIncrease.."|r|cFFFFFFFF "..L["bonus"].." =|r|cFF69FF69 "..(LEAM+LEAMIncrease) -- Bônus da profissão
	...
	local maxtext = "\n"..L["maxtext"]..TitanUtils_GetHighlightText(LEAMmax)

	local ColorValueAccount -- Conta de ganho de perícia
	if not LEAM then
		ColorValueAccount = ""
	elseif LEAM == GetMaxProfessionCap() then
		ColorValueAccount = "\n"..L["maxskill"]
